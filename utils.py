...
def get_headless_chrome(user_agent):
...
def find_path(name):
    for root, dirs, files in os.walk(os.getcwd()):
        if name in files or "{}.exe".format(name) in files:
            return os.path.join(root, name)

    logging.error('Could not locate the {} file, make sure the file is in the adidas-waiting-room-festivus folder'.format(name))
